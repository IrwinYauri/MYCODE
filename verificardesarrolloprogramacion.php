<?php 
	...
	$concursante=str_replace(' ', '', trim(preg_replace('/\s+/', ' ', str_replace(' ', '', $_POST["outputrpt_".$_POST["indice"]]))));//setear("ingreso.java");
	$concursante=str_replace('<br>', '', $concursante);

	if($maestro==$concursante){
		
		$sql = "INSERT INTO ejercicio_prueba (codpru,codeje,codigo) values(".$_POST["codpru"].", ".$_POST["codeje"].", '".$_POST["textareaCode".$_POST["indice"]]."')";  
    	$conn->query($sql);
